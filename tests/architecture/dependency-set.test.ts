...
// the command-partition scanner would (correctly) flag while scanning tests/**.
...
function cloneRepo(): string {
  ...
  expect(
    spawnSync('git', ['clone', '--quiet', REPO, clone], { encoding: 'utf8' })
      .status,
  ).toBe(0);
  return clone;
}
function commitFixture(clone: string, relPath: string, content: string): void {
  ...
  spawnSync(
    'git',
    ['-C', clone, 'commit', '-q', '-m', 'fixture', '--no-gpg-sign'],
    {
      encoding: 'utf8',
      env: {
        ...process.env,
...
function runCheckDeps(clone: string): { status: number | null; out: string } {
  const r = spawnSync(
    process.execPath,
    [join(clone, 'tools/check-dependencies.mjs')],
    { cwd: clone, encoding: 'utf8' },
  );
  return { status: r.status, out: r.stdout + r.stderr };
}

interface CurrentCheckResult {
  status: number | null;
  stdout: string;
  stderr: string;
}

let dependencyCheck: CurrentCheckResult | undefined;
function currentDependencyCheck(): CurrentCheckResult {
  dependencyCheck ??= spawnSync(
    process.execPath,
    [join(REPO, 'tools/check-dependencies.mjs')],
    { cwd: REPO, encoding: 'utf8' },
  );
  return dependencyCheck;
}

let boundaryCheck: CurrentCheckResult | undefined;
function currentBoundaryCheck(): CurrentCheckResult {
  boundaryCheck ??= spawnSync(
    process.execPath,
...
describe('dependency partition', () => {
  it('the shipped runtime uses exactly the two locked external packages', () => {
    const r = currentDependencyCheck();
    const out = JSON.parse(r.stdout) as { used_external: string[] };
    expect(out.used_external).toEqual(
      expect.arrayContaining(['ajv', 'better-sqlite3']),
    );
  });

  it('the product boundary closure resolves entirely locally', () => {
    const r = currentBoundaryCheck();
  ...
  it('helper/CLI partition names the toolchain-preflight system helpers (clang++/xcode-select/xcrun)', () => {
    const r = currentDependencyCheck();
    const out = JSON.parse(r.stdout) as {
      used_commands: string[];
      errors: string[];
    };
    expect(out.errors).toEqual([]);
    expect(out.used_commands).toEqual(
      expect.arrayContaining(['clang++', 'xcode-select', 'xcrun']),
    );
  });
  ...
  it('live tools and tests do not hard-code host-specific Homebrew Git or npm paths', () => {
    ...
    expect(
      listed.status,
      listed.stderr?.toString('utf8') ?? listed.error?.message,
    ).toBe(0);
    const hostPrefix = ['/usr', 'local', 'bin'].join('/');
    const forbidden = new RegExp(`${hostPrefix}/(?:git|npm)\\b`);
    const executableSources = listed.stdout
      .toString('utf8')
      .split('\0')
      .filter((path) => /^(src|tools|tests)\/.*\.(?:ts|mts|mjs)$/.test(path))
      .filter((path) => existsSync(join(REPO, path)));
    for (const path of executableSources) {
      expect(readFileSync(join(REPO, path), 'utf8'), path).not.toMatch(
        forbidden,
      );
    }
  });

  it('keeps the reviewed computed-command owners explicit', () => {
    const manifest = JSON.parse(
      spawnSync(
        'git',
        ['-C', REPO, 'show', 'HEAD:provenance/dependency-toolchain.v1.json'],
        {
          encoding: 'utf8',
        },
      ).stdout,
    ) as { computed_command_owners: string[] };
  ...
  it('OMISSION fixture: dropping a helper row from the manifest fails the checker', () => {
    ...
    const m = JSON.parse(
      spawnSync('git', ['-C', clone, 'show', `HEAD:${manifestPath}`], {
        encoding: 'utf8',
      }).stdout,
    ) as {
  ...
  it('N2 fail-closed: a computed spawn with no proven tuple binding fails even when the file has an unrelated tuple', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-computed-tuple.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n` +
  ...
  it('N2 array-construction misclassification: a computed spawn whose variable only appears in an ordinary array literal fails', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-array-construction.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n` +
        `const c = process.env.SNEAKY;\n` +
  ...
  it('N2 exact arbitrary-RHS destructuring evasion fails closed', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-arbitrary-rhs.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n\n` +
  ...
  it('N2 exact same-name shadowing evasion from the fresh review fails closed', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-same-name-shadow.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n\n` +
        `const command = 'git';\n` +
        `{\n` +
        `  const attackerControlled = [process.env.SNEAKY, []];\n` +
  ...
  it('N2 reassigned let command tokens fail closed even when initialized to an allowed literal', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-reassigned-let.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n` +
        `let command = 'git';\n` +
  ...
  it('N2 aliased named child_process imports cannot hide an undeclared executable edge', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-aliased-import.mjs',
  ...
  it('N2 child_process namespace member calls cannot hide an undeclared executable edge', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-namespace-member.mjs',
  ...
  it('N2 binding syntax alone cannot authorize even a literal tuple in a non-owner file', () => {
    ...
    commitFixture(
      clone,
      'tools/fixture-literal-destructure.mjs',
      `import { ${SPAWN} } from 'node:child_process';\n` +
