...
`);
    execFileSync("/usr/bin/xcrun", ["swiftc", "-swift-version", "5", "-parse-as-library", "-warnings-as-errors",
      ...
      join(root, "overlay.swift"), join(repo, "product/echo-overlay/ui-support.swift"), join(repo, "product/echo-overlay/people.swift"), join(repo, "product/echo-overlay/account.swift"), join(repo, "product/echo-overlay/uploads.swift"), join(repo, "product/echo-overlay/projects.swift"), join(repo, "providers/slack/client/swift/slack-connected-tools.swift"), join(root, "proof.swift"), "-o", binary],
