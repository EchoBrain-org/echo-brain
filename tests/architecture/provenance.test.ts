...
describe('provenance records', () => {
  it('each provenance doc validates against its committed schema', () => {
    const pairs: Array<[string, string]> = [
      [
        'provenance/extraction-policy.v1.json',
        'provenance/schemas/extraction-policy.v1.schema.json',
      ],
      [
        'provenance/source-plan.v1.json',
        'provenance/schemas/source-plan.v1.schema.json',
      ],
      [
        'provenance/source-extraction.v1.json',
        'provenance/schemas/source-extraction.v1.schema.json',
      ],
      [
        'provenance/test-parity.v1.json',
        'provenance/schemas/test-parity.v1.schema.json',
      ],
      [
        'provenance/dependency-toolchain.v1.json',
        'provenance/schemas/dependency-toolchain.v1.schema.json',
      ],
    ];
    for (const [doc, schema] of pairs) {
      ...
      expect(
        validate.errors ?? [],
        `${doc}: ${JSON.stringify(validate.errors)}`,
  ...
  it('reviewed policy has empty exclusion allowlist, only the adjudicated package.json transform, and the 21-path target-only set', () => {
    const policy = readJson('provenance/extraction-policy.v1.json') as {
      dispositions: {
        transform_allowlist: Array<{ target_path: string; transform: string }>;
        exclusion_allowlist: unknown[];
      };
    ...
    expect(policy.dispositions.transform_allowlist[0].target_path).toBe(
      'package.json',
    );
    expect(policy.dispositions.transform_allowlist[0].transform).toMatch(
      /engines\.npm/,
    );
  ...
  it('detects disposition evasion (tampered target blob) in a working copy', () => {
    ...
    const victim = join(clone, 'src/core/index.ts');
    writeFileSync(victim, readFileSync(victim, 'utf8') + '\n// tamper\n');
    spawnSync(
      'git',
      ['-C', clone, 'commit', '-am', 'tamper', '--no-gpg-sign'],
      {
        encoding: 'utf8',
        env: {
          ...process.env,
