...
class FakeOutbox implements IndependentCopyOutboxSource {
  ...
  readonly eventHashes = new Map<FederationId, Map<number, Sha256Digest>>();
  ...
  function diskInfoFixture(
    ...
    const values = new Map<string, Record<string, unknown>>([
      ...
      [
        targetRoot,
        {
          ...
          APFSPhysicalStores: [{ APFSPhysicalStore: targetPhysicalStore }],
        },
      ],
      [
        targetWholeDisk,
    ...
    const inspector = new MacOsEncryptedVolumeInspector({
      ...
      readDiskUtilityInfo: async (path) => {
        ...
        if (value === undefined)
          throw new Error(`missing disk fixture ${path}`);
        return value;
      },
    });
