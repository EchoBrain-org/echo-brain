...
export class CountingInstallationSigner implements InstallationSigner {
  ...
  constructor(private readonly installationId = federationFixtureId('ins', 1)) {
...
export function federatedApprovalGroupDrafts(
  ...
  const organizationId =
    options.organizationId ?? federationFixtureId('org', 1);
  ...
  const sourceBindingId =
    options.sourceBindingId ?? federationFixtureId('bnd', 1);
  ...
  return signals.map((signalId, index) => {
    ...
    const envelope = {
      ...
      identity_manifest_sha256: options.manifestSha256 ?? options.digests.a,
