...
const POISON_FIXTURES = [
  ...
  ["raw nonce on challenge ticket", ["challenge", "raw_nonce"], "one-time-secret"],
  [
  ...
  ["signing private key", ["signing_key", "private_key"], "private-key-material"],
  ...
  [
    ...
    ["commit", "plan", "registry", "connections", 0, "generations", 0, "provider_identity", "raw_response"],
  ...
  ["meeting content on the completion result", ["result", "meeting"], { title: "Not completion evidence" }],
...
describe("Founder bootstrap persisted-session exact shape", () => {
  ...
  it.each([
    ...
    [
      "audience count",
      ["request", "publication", "audience", "subjects"],
      [],
    ],
    [
    ...
    [
      ...
      ["commit", "plan", "manifest", "legacy_cutover", "native_records_require"],
      [],
    ],
  ] satisfies readonly MutationCase[])("bounds the Founder Live %s", (_name, path, value) => {
    const session = cloneSession();
    setAtPath(session, path, value);
    expect(() => assertExactFounderBootstrapSessionShape(session)).toThrow(
