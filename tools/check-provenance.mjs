...
// additionally exercised by tests/architecture/provenance.test.ts (ajv).
//
...
const EXPECTED_SPEC_SHA =
  '832be81341f2c523fd42918206774ec8a51f54de653a323ad56d612e0ea47748';
...
function selectedCommit(argv) {
  ...
  if (argv.length === 2 && argv[0] === '--commit' && argv[1] !== '')
    return argv[1];
...
function main() {
  ...
  if (policy.reviewed_spec_sha !== EXPECTED_SPEC_SHA)
    errors.push(`reviewed_spec_sha mismatch: ${policy.reviewed_spec_sha}`);
  if (policy.source.source_sha !== EXPECTED_SOURCE_SHA)
    errors.push(`policy source_sha mismatch: ${policy.source.source_sha}`);
  if ((policy.dispositions.exclusion_allowlist ?? []).length !== 0)
    errors.push('exclusion_allowlist not empty');
  ...
  for (const t of transformAllow) {
    if (t.target_path !== 'package.json' || !/engines\.npm/.test(t.transform)) {
      errors.push(
        `unexpected transform_allowlist entry: ${t.target_path} (${t.transform})`,
      );
    }
  }
  const declaredTargetOnly = new Set(policy.target_only_paths);
  if (declaredTargetOnly.size !== 21)
    errors.push(`target_only_paths size ${declaredTargetOnly.size} != 21`);

  const extraction = get(SELF);
  if (extraction.reviewed_spec_sha !== EXPECTED_SPEC_SHA)
    errors.push('source-extraction reviewed_spec_sha mismatch');
  if (extraction.source_sha !== EXPECTED_SOURCE_SHA)
    errors.push('source-extraction source_sha mismatch');
  ...
  for (const e of extraction.entries) {
    if (partitioned.has(e.target_path))
      errors.push(`duplicate partition entry: ${e.target_path}`);
  ...
  for (const [path, { mode }] of [...tree].sort()) {
    ...
    if (!e) {
      errors.push(`unpartitioned target blob: ${path}`);
      continue;
    }
    if (e.git_mode !== mode)
      errors.push(`mode drift ${path}: ${e.git_mode} != ${mode}`);
  ...
  for (const extra of partitioned.keys())
    errors.push(`partition entry for absent/self blob: ${extra}`);
  ...
  for (const p of declaredTargetOnly)
    if (!actualTargetOnly.has(p))
      errors.push(`declared target-only not partitioned as target-only: ${p}`);
  for (const p of actualTargetOnly)
    if (!declaredTargetOnly.has(p))
      errors.push(`partitioned target-only not declared: ${p}`);
  ...
  for (const e of extraction.entries) {
    if (e.origin.kind === 'copied' && e.origin.source_path !== e.target_path)
      errors.push(`copied path mismatch ${e.target_path}`);
    if (e.origin.kind !== 'target-only' && !e.origin.source_blob_oid)
      errors.push(`missing source_blob_oid ${e.target_path}`);
    if (e.origin.transform) {
      transformedInExtraction.add(e.target_path);
      if (!allowByPath.has(e.target_path))
        errors.push(
          `transformed origin ${e.target_path} absent from transform_allowlist`,
        );
    }
  }
  for (const p of allowByPath.keys()) {
    if (!transformedInExtraction.has(p))
      errors.push(
        `transform_allowlist entry ${p} has no transformed source-extraction origin`,
      );
  }
