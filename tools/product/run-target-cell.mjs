...
export function runTargetCell({ artifactDir, supportDir, workDir }) {
  ...
  try {
    ...
    mkdirSync(dirname(operatorConfig), { recursive: true });
    const onboarded = runCli(
      firstBin,
      ['onboard', '--config', operatorConfig, '--state-dir', operatorState],
      workDir,
    );
    evidence.statuses.onboard = onboarded.ok;
    if (!onboarded.ok) throw new Error(`onboard failed: ${onboarded.stderr}`);
