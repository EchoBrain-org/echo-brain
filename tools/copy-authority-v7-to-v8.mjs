...
import { copyAuthorityV7ToV8 } from '../packages/organization-authority-kernel/dist/adapters/persistence/sqlite/authority-v7-to-v8.js';

const args = process.argv.slice(2);
if (args.length !== 2) throw new Error('usage: node tools/copy-authority-v7-to-v8.mjs <stopped-snapshot-authority.sqlite> <absent-output.sqlite>');
const sourcePath = resolve(args[0]);
const outputPath = resolve(args[1]);
if (existsSync(outputPath)) throw new Error('offline output must not already exist');
const staging = mkdtempSync(join(dirname(outputPath), '.authority-v8-'));
...
try {
  ...
  process.stdout.write('Authority V8 snapshot created; input preserved. Live conversion and release remain separate operator work.\n');
} finally {
