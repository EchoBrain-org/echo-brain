...
before local exercise, onboarding, or deploy. Do not add a tool-specific
procedure.
...
The synthetic `./update-clean-v1.sh canary` path was staging-only and was
retired with the AWS staging slot on 2026-09-06. A deployment needs durable
progress from its admitted live source before it can become terminal green.
...
The installed wrappers hold a root-owned `.staging-release-guard` outside
`clean-data` as an additional interlock. Preserve it and the original operation
receipt if execution is unconfirmed or reports `control_path_changed`. The
legacy-lock cleanup below does not authorize removing this guard; investigate
the exact command and pinned control-state identity first.
...
exact reviewed `backup-authority-maintenance.sh` is installed at
`/srv/echo-authority-clean-v1/backup-authority-maintenance.sh`, owned by root
with mode `0755`, and its SHA-256 is verified against the private review
receipt.
