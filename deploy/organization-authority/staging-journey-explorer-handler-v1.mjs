/** Creates a bounded reader from the selected providers' historical vocabulary. */
export function createStagingJourneyExplorerModuleV1({ providers = [], models = [], legacy_phases = [] } = {}) {
  for (const [values, max] of [[providers, 7], [models, 15], [legacy_phases, 16]]) {
    if (!Array.isArray(values) || values.length > max || values.some(value =>
      typeof value !== 'string' || value.length > 128 || !/^[a-z0-9][a-z0-9/_.-]*$/.test(value))) {
      throw new Error('Explorer vocabulary is invalid');
    }
  }
...
const PROVIDERS = new Set([...providers, 'other']);
...
async function sdk() {
  return import("@aws-sdk/client-cloudwatch-logs");
}
...
return Object.freeze({ createStagingJourneyExplorerHandlerV1, parseRequestV1: request, summarizeJourneyEventsV1: summarize, timelineV1: timeline, handler });
}
