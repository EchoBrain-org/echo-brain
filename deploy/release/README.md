...
The direct host commands below are the installed-wrapper contract, run by a
human through Session Manager; they are not permission for an agent to open a
shell session. The automated current-host staging lane was retired with the
AWS staging slot on 2026-09-06.
...
host runs the installed wrapper:
