  ...
  for (;;) {
    ...
    if (created) {
      ...
      try {
        ...
        throw new ProcessFileLockError('io', 'cannot inspect new process lock', {
          cause: error,
        });
      }
    ...
    try {
      ...
      if (errno(error) === 'ENOENT') continue;
      throw new ProcessFileLockError('io', 'cannot inspect process lock owner', {
        cause: error,
      });
    }
    if (entries.length === 0) {
    ...
    const match = /^(\d+)-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.exec(
      entries[0]!,
    );
    if (match === null) {
      throw new ProcessFileLockError(
    ...
    if (
      ...
      try {
        ...
        if (errno(error) === 'ENOENT') continue;
        throw new ProcessFileLockError('io', 'cannot recover stale process lock', {
          cause: error,
        });
      }
      try {
