...
/**
 * Rehearsal-only fault injection. Tampers with a persisted access-state
 * document so a subsequent read fails closed with an
 * {@link OrganizationStateCorruptionError}. It lives in the owning module so a
 * schema change to organization_access_high_watermarks is reconciled here
 * rather than silently diverging from the Phase 5 driver that exercises it.
 */
