...
export interface InstallationSigner {
  ...
  deleteOrphan?(installationId: string): Promise<boolean>;
}
...
export function verifyInstallationKeyDescriptor(
  ...
  if (
    (descriptor.protection === 'secure-enclave' && descriptor.assurance !== 'hardware_bound') ||
  ...
  const publicKey = Buffer.from(descriptor.public_key_spki_der_base64, 'base64');
  if (publicKey.length === 0 || publicKey.toString('base64') !== descriptor.public_key_spki_der_base64) {
    throw new Error('installation public key is not canonical base64');
  }
  if (p256KeyId(publicKey) !== descriptor.key_id) {
    throw new Error('installation key fingerprint does not match its public key');
  }
  ...
  if (!Buffer.isBuffer(canonicalSpki) || !canonicalSpki.equals(publicKey)) {
    throw new Error('installation public key must use canonical P-256 SPKI DER bytes');
  }
  return publicKey;
}
...
export async function signWithInstallationKey(
  ...
  if (descriptor.key_id !== expectedKeyId) {
    throw new Error('installation signing key does not match the active identity');
  }
