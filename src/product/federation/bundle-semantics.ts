...
export function validateIdentityDocumentSemantics(
  ...
  for (const claim of manifest.identity_claims) {
    ...
    if (
      ...
      fail('Slack challenge claim method, issuer, subject, or assurance disagrees');
    }
    if (
  ...
  const connectionById = new Map(
    registry.connections.map((connection) => [connection.connection_id, connection]),
  );
  for (const connection of registry.connections) {
    if (connection.organization_id !== organizationId) {
      fail(`connection ${connection.connection_id} belongs to another organization`);
    }
    if (
    ...
    if (
      connection.generations.filter((generation) => generation.ended_at === null)
        .length > 1
    ) {
      fail(`connection ${connection.connection_id} has multiple active generations`);
    }
  ...
  for (const binding of registry.bindings) {
    if (
      ...
      fail(`binding ${binding.adapter_binding_id} configuration digest disagrees`);
    }
    ...
    if (connection === undefined || generation === undefined) {
      fail(`binding ${binding.adapter_binding_id} refers to a missing connection generation`);
    }
    if (binding.status === 'active' && generation.ended_at !== null) {
      fail(`binding ${binding.adapter_binding_id} refers to a retired credential generation`);
    }
    const provider = expectedProvider(binding.adapter_id);
    if (provider !== null && connection.provider !== provider) {
      fail(`binding ${binding.adapter_binding_id} uses the wrong provider connection`);
    }
  }

  const slackTenants = new Set(
    registry.connections
      .filter((connection) => connection.provider === 'slack')
      .flatMap((connection) =>
        connection.generations.flatMap((generation) =>
          generation.ended_at === null &&
          generation.provider_identity.tenant !== null
            ? [generation.provider_identity.tenant.id]
            : [],
        ),
      ),
  );
  for (const claim of manifest.identity_claims) {
    if (
      claim.issuer.kind === 'provider' &&
      claim.issuer.provider === 'slack' &&
      !slackTenants.has(claim.issuer.tenant_id)
    ) {
      fail('Slack human claim and bot connection name different workspaces');
    }
  }
...
export function validateRegistryAgainstRuntime(
  ...
  for (const expected of configured) {
    ...
    if (
      ...
      fail(`runtime settings drifted from binding ${binding.adapter_binding_id}`);
    }
    if (binding.connection_id === null) {
      if (expected.config.credential_ref !== undefined) {
        fail(`credentialed adapter ${binding.adapter_binding_id} has no connection`);
      }
      continue;
    }
    ...
    if (generation === undefined) {
      fail(`binding ${binding.adapter_binding_id} has no credential generation`);
    }
    if (
      ...
      fail(`runtime credential reference drifted from binding ${binding.adapter_binding_id}`);
    }
  }
}
