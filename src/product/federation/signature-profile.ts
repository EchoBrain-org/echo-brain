...
function readInteger(bytes: Buffer, offset: number): {
  value: bigint;
  next: number;
} {
  if (bytes[offset] !== 0x02) throw new Error('ECDSA signature integer tag is invalid');
  const length = bytes[offset + 1];
  if (length === undefined || length === 0 || length > 33 || (length & 0x80) !== 0) {
    throw new Error('ECDSA signature integer length is invalid');
  }
  const start = offset + 2;
  const end = start + length;
  if (end > bytes.length) throw new Error('ECDSA signature integer is truncated');
  ...
  const value = magnitude.length === 0 ? 0n : BigInt(`0x${magnitude.toString('hex')}`);
...
export function decodeStrictP256DerSignature(
  ...
  if (length === undefined || (length & 0x80) !== 0 || length !== signature.length - 2) {
  ...
  if (r.value <= 0n || r.value >= P256_ORDER || s.value <= 0n || s.value >= P256_ORDER) {
...
export function assertP256LowS(signature: Buffer): void {
  ...
  if (s > P256_HALF_ORDER) throw new Error('ECDSA P-256 signature is not low-S');
}
