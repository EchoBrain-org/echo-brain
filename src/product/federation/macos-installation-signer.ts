...
function parseHelperResponse(raw: string): HelperResponse {
  ...
  try {
    ...
    throw new Error(`installation signer returned invalid JSON: ${(error as Error).message}`);
  }
...
function requireDescriptor(
  ...
  if (descriptor.installation_id !== installationId) {
    throw new Error('installation signer returned a descriptor for another installation');
  }
...
export class MacOsSecureEnclaveInstallationSigner implements InstallationSigner {
  ...
  async sign(
    ...
    if (expectedKeyId === undefined) {
      throw new Error('installation signing requires the expected key fingerprint');
    }
    const response = await invokeHelper({
      schema_version: 1,
      command: 'sign',
    ...
    if (encoded === undefined) throw new Error('installation signer omitted its signature');
    const signature = Buffer.from(encoded, 'base64');
    if (signature.length === 0 || signature.toString('base64') !== encoded) {
      throw new Error('installation signer returned non-canonical signature base64');
    }
    return signature;
  }
}
