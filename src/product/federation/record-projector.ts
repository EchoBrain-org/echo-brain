...
import {
  ...
  type FederatedChainVerificationKeyResolver,
...
function signals(state: DecisionNodeState) {
  return [
...
/** Projects one already-validated immutable approval node into its signed group. */
export class FederatedRecordProjector {
  ...
  async projectApproved(
    ...
    const firstSubject = `approved-org-record:${state.approval_id}:${items[0]!.signal.id}`;
    ...
    const approval = snapshots.approval;
    const signalManifest = items.map(({ signal, position }) => ({
      signal_id: signal.id,
      kind: signal.kind,
    ...
    const loadHistoricalManifestByDigest =
      this.options.lineage.loadVerifiedManifestBySha256;
    if (loadHistoricalManifestByDigest === undefined) {
      fail('identity lineage cannot resolve historical manifest digests');
    }
    const historicalVerificationKeyResolver: FederatedChainVerificationKeyResolver =
      (event) => {
    ...
    const drafts: FederatedOutboxEventDraft[] = items.map(({ signal }) => {
      const record = {
        ...
        signal_id: signal.id,
        signal: clone(signal),
        meeting_context: clone(state.brief.meeting),
        approval_group: {
          brief_schema_version: 1 as const,
          brief_id: state.brief.id,
          approved_brief_sha256: approvedBriefSha256,
          signal_manifest: clone(signalManifest),
        },
      ...
      const envelope: FederatedEventDraftV1 = {
        ...
        producer: {
          ...
          product_artifact: clone(projectionArtifact),
        },
        source: clone(snapshots.source),
        processor: clone(snapshots.processor),
        local_reference: {
          processing_key: state.processing_key,
          approval_id: state.approval_id,
          node_id: state.node_id,
          meeting_id: state.brief.meeting.id,
          signal_id: signal.id,
        },
        record,
        ...
        identity_manifest_sha256: verifiedManifest.sha256,
      };
      return {
        local_subject_key: `approved-org-record:${state.approval_id}:${signal.id}`,
        envelope,
      };
    });
