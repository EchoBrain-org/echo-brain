...
export function extractLinuxUpdateKit(archive: string, destination: string): void {
  let entries: string[];
  try {
...
export function validateLinuxUpdateRelease(kit: string, manifest: ClientUpdateManifest): void {
...
export function installLinuxClientUpdate(input: UpdateInstallerInput, environment = process.env): void {
  ...
  try {
    ...
    extractLinuxUpdateKit(input.archive, kit);
    validateLinuxUpdateRelease(kit, input.manifest);
    const result = spawnSync('/bin/bash', [join(kit, 'Start-ECHO.sh'), '--install-only', '--expected-wrapper-sha256', input.expected_wrapper_sha256], {
      ...
      env: { ...environment, XDG_DATA_HOME: join(input.root, '../..') },
