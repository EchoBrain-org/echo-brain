...
All durable identity records live below the existing private `state_dir`. One
deliberately data-minimal cutover guard lives beside it so replacing or losing
the entire state tree cannot silently return the profile to pre-cutover mode:
...
<parent-of-state_dir>/
  .echo-founder-cutover.<state-path-hash>.v1.json
```

The SQLite file receives only additive tables described below. No existing rows or primary keys are rewritten.

The sibling guard contains only the bootstrap session ID, plan digest,
installation/key IDs, and state-path digest. It contains no
credential, private key, provider evidence, meeting content, or publication
payload. It is outside product backups by design and is checked before a
pre-cutover restore can replace state. A future central enrollment authority
can become the stronger durable fence without changing any signed record or
local identity ID.
...
A local outbox is durable against process crashes, not against loss of the only machine. Until a server exists, continued Founder Live requires a protected, verified independent signed-outbox export after native approvals. A normal state backup alone does not satisfy this gate. A plaintext export in a public or broadly shared folder is a policy violation because it contains internal evidence and participant observations. Claiming machine-loss durability without a protected second copy would be false.

For the first local implementation, “independent” is deliberately concrete: the target must be a mounted, platform-verified encrypted volume backed by an external, non-virtual physical storage device that does not underlie `state_dir`. A second mode-`0700` folder, APFS volume, partition, internal drive, sparsebundle, or disk image on the laptop does not qualify. Echo records the target volume identity, writes a full signed outbox-prefix export for every current chain head, reads and cryptographically verifies that export back from the target, and stores an immutable local intent/receipt. The target's verified full-prefix history is also monotonic: a restored or cloned local outbox may not fall behind or fork from the newest protected export. A native approval cannot return to core—and therefore cannot deliver or advance its cursor—until this copy is current. With an empty outbox, cutover requires the same target/binding read-back proof even though there is not yet an event to copy.
...
`legacy_imported_unverified` is reserved for an already-delivered record that must remain as historical local evidence. In this minimum, `disposable_test` means permanently ineligible for export or promotion, not immediately deletable: its local identity marker and bytes remain frozen so the same processing key and approval ID cannot later be recreated as native. A future controlled discard may replace its content with a durable non-reusable tombstone, but raw deletion is not safe. Both classes are excluded from the signed federated outbox and future organization brain. Echo does not offer a later retrospective-promotion ceremony.
...
9. Require every legacy node to be non-pending and durably processed, commit the immutable legacy report, configure and read back the protected export target, then write the sibling cutover guard before moving the signed bootstrap session to `committing`.
10. Fsync/install the staged signed manifest, registry revision, and policy; atomically rename the signed active-bundle pointer **last**; verify the independent signed-outbox copy and strict identity check; then sign the completed bootstrap receipt. Only that green result qualifies later records as native Founder Live records.

The production commit command therefore takes `--independent-copy-root <absolute-path>`. After cutover, `echo-brain export --config <absolute-path>` is the idempotent repair/reverification command when a target was temporarily unavailable; it does not create a second record or advance the outbox.

Before the irreversible guard is written, every structurally legacy decision node must already be non-pending and have the core's durable processed marker. This prevents a pre-cutover approval from remaining behind the Granola cursor with no legal approve, reject, or promotion path after activation. The guard is a private canonical sibling of `state_dir`, written before the signed session enters `committing`; it keeps the profile fail-closed if the state tree is lost or a supported restore tries to load a pre-cutover backup. Restore may use an older identity-active backup only when its signed cutover session names the same bootstrap session, plan, and installation key; the external export history then detects any outbox rollback.
...
Bootstrap is resumable. Before the sibling guard is written, a crash may leave only unreachable immutable staged files and an orphan Keychain key; retry verifies and reuses or explicitly deletes that named orphan before minting another. Once the guard is written, including a crash before the active-pointer rename, the profile is irreversibly identity-enabled and fails closed until the same signed session resumes and completes. A crash after the rename exposes the entire coherent bundle. Connection rotation, policy changes, and re-enrollment use the same “write immutable dependencies, pointer last” commit rule.
...
8. A protected independent signed-outbox export can be read and cryptographically checked before Founder Live continues; a normal state backup alone does not qualify.
...
- Create a durable sync/export outbox and verify a protected independent signed-outbox export so the laptop is not the only promotable copy; a normal state backup alone does not qualify.
