...
| Control-plane `clean-runtime-v1`, `clean-slack-identity-v1`, `clean-founder-v1`, and `new-lineage-genesis-v1` exports | Slack approval integration, external identity, connection setup, and control database | Existing package consumers import these aliases. Keep the responsibility-named entrypoints canonical and remove aliases only in a versioned package migration. |
