...
| Authority `clean-{reset,person,live,granola-source}-main` files, legacy `echo-organization-authority-*clean*` binaries, and `clean-v1` deployment paths | Authority state bootstrap, Person administration, service lifecycle, meeting-source admission, and deployment | Installed scripts, package binaries, container commands, and host paths call these names. Responsibility-named binaries already exist where practical; retire the aliases and deployed paths only through a bounded release migration. |
| Control-plane `clean-runtime-v1`, `clean-slack-identity-v1`, `clean-founder-v1`, and `clean-person-slack-*-activate-main` exports or binaries | Slack approval integration, external identity, connection setup, and legacy reaction-approval activation | Existing package consumers and operator commands import or invoke these aliases. Keep the responsibility-named entrypoints canonical and remove aliases only in a versioned package/operations migration. |
