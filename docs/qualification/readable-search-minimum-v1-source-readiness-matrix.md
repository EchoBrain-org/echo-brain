...
reviewed_ref: 6b9f925908b7790c008f81147c3a6837cc63083a
matrix_version: 1
assertion_ids:
  - RSMV1-001
  - RSMV1-002
...
serving Layer 3 authorization/audit. It records the source and focused-test
surfaces that must be evidenced by a future exact run.

It is not a qualification report. The referenced implementation and focused
tests are presently uncommitted working-tree material, so this matrix makes no
`source-tested`, artifact-tested, deployed, founder-live-qualified,
client-live-qualified, restore-qualified, or released claim. In particular,
the existing Job A stopped-state and Job B active-member reports remain their
own immutable evidence and do not satisfy every assertion below.

The `reviewed_ref` identifies the committed documentation baseline only. A
future report must name the exact source commit, artifact, configuration,
state generation, per-assertion outcome, and evidence IDs. Restore and
founder-live assertions require separate exact-run evidence; neither may be
recorded as passed from local source inspection or focused tests.
...
| `RSMV1-015` | Internally consistent stale state remains offline until independently retained heads/receipts, Person state, policy facts, record head, active pointer, manifests, roots, and audit storage reconcile. | Restore operational gate | Required separate restore-reconciliation exact run. The local operator-state and stopped-build source/test surfaces do not qualify this row; no restore qualification report is linked here. |
...
| `RSMV1-018` | A real reviewer-v2 record and a real organization-v3 record survive Authority restart and stopped generation rebuild; the exact reviewer sees both, a separately enrolled later member sees only the organization item, and no hidden segment/content access occurs. | Founder-live operational gate | Required separate founder-live exact run. No founder-live qualification report is linked to this matrix; existing Job A and Job B reports retain only their declared scopes. |
