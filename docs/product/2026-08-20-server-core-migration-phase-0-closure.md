...
| `P0-P-006` | Processing candidates, slots, approval contracts/publications, resolution proof, processed markers, frozen act input, delivery intent/attempt/receipt meaning                                     | Durable pre-record work, exact human outcome, crash recovery, terminal cleanup basis, and separate delivery acknowledgement.                                                                | `PROCESSING`             | [T05], [T06], [T07], [T11] |
...
| `P0-R-005` | Installation-signed permission request/verifier files and V1 Slack identity-link transport                                                                                                                                                    | Server-owned approval action rechecks the current exact link, connection, binding, capability, provider act, policy, and membership; Person link completion remains link-only.                                                                           | D2, D5                                         | `IDENTITY`               | [T03], [T05], [T13]               |
| `P0-R-006` | `SlackIdentityLinkInstallation`, installation begin/complete branches, and installation ownership fields in adapter bindings/capabilities                                                                                                     | Person link plus separate owner/admin approval activation; only ownership columns disappear, never stable provider/adapter/ECHO edges.                                                                                                                   | D2, D5                                         | `IDENTITY`               | [T03], [T05], [T13]               |
| `P0-R-007` | `organization_integration_audit.actor_installation_id`, installation actor kind, and the old entry preimage/digest                                                                                                                            | New domain-separated, lineage-bound audit entry reconstructs the same commitments for both policies and rejects cross-version substitution.                                                                                                              | D2                                             | `IDENTITY`, `RECORD`     | [T03], [T05], [T06]               |
| `P0-R-008` | Installation-signed `record-envelope.ts`, `record-envelope-v2.ts`, `record-envelope-v3.ts`, `record-receipt.ts`; `/v1/record-envelopes`; `organization_record_log.installation_id` and installation-scoped dedupe                             | Authority-writer envelope/receipt/log version freezes exact human act, identity proof, provenance, policy, delivery intent, semantic idempotency, and approval-only facts.                                                                               | D1, D3-D5                                      | `RECORD`                 | [T06], [T08], [T13]               |
...
| `P0-R-016` | Former rejection terminality before record-first resolution and approval-only frozen envelope sidecar                                                                                                                                        | Phase 2C-A now writes the existing V1 canonical rejection before terminal marking, with no delivery, and widened frozen-envelope storage without rewriting old rows. New-lineage rejection receipt/fact semantics remain D3/D4 work.                     | Compatibility ordering complete; D3/D4 pending | `PROCESSING`, `RECORD`   | [T06], [T08]                      |
...
| Delivery binding, destination, contract, and approved snapshot         | Approval freezes intent; delivery rechecks current activation before call      | Canonical delivery intent plus durable attempt/receipt                                           | Binding may revoke for future calls; frozen intent does not change | Yes              | Attempt details may remain outside Layer 1         | Preserve separately from approval                         |
...
| D6   | Accepted shared Person-read audit shape, operation-specific scope commitments, retention, export position, and audit-before-byte protocol                                                                                                                                               | Route-specific audit tables, repositories, exporters, expiry code, and CLIs                               | Open; implementation may proceed additively |
