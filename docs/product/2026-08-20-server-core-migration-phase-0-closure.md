...
| `P0-R-008` | Installation-signed `record-envelope.ts`, `record-envelope-v2.ts`, `record-envelope-v3.ts`, `record-receipt.ts`; `/v1/record-envelopes`; `organization_record_log.installation_id` and installation-scoped dedupe                             | D3-1 privately freezes the exact human-act leaf graph and semantic idempotency. Later Authority-writer envelope/receipt/log versions must add exact identity proof, provenance, policy, signatures, and approval-only facts while delivery remains a separate post-record state machine. | D3-1 candidate green; D1, D3-D5 pending        | `RECORD`                 | [T06], [T08], [T13]               |
