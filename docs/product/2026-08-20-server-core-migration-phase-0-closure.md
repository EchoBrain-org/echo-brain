...
| `P0-R-008` | Installation-signed `record-envelope.ts`, `record-envelope-v2.ts`, `record-envelope-v3.ts`, `record-receipt.ts`; `/v1/record-envelopes`; `organization_record_log.installation_id` and installation-scoped dedupe                             | D3-1 freezes the exact human-act leaf graph and semantic idempotency. D3-2 privately freezes the Authority/lineage-pinned v4 envelope, exact source/processor provenance, record hash, and detached signature while leaving opaque processor-contract reproof, receipt/log/facts, persistence, and live selection to D3-3/Phase 3. | D3-1/D3-2 candidates green; D1, D3-D5 pending | `RECORD`                 | [T06], [T08], [T13]               |
