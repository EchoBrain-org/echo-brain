...
| `P0-R-013` | Former `process-one-meeting.ts`, `baseline-live-source.ts`, `run-one-meeting.ts`, structured-text live composition, canary-only store APIs, and retained production synthetic replay                                                                 | Phase 2B replaced the live canary with atomic stopped `activate-meeting-source` and a complete processing key. The Phase-1 replay corpus is disposed on schedule 2026-08-25 by the 2026-08-20 corpus disposition; the D5 parity corpus is authored fresh in Phase 3.                                             | Live canary complete; Phase-1 corpus disposal due 2026-08-25 | `PROCESSING`             | [T04], [T13], [T14]               |
...
| `P0-R-022` | Current 29 Authority, control-plane, record-log, and derived migration files in the runtime migration closure                                                                                                                                 | D0-authorized fresh exact baselines with role-stable application IDs, fresh lineage manifests, new envelope/receipt/log versions, and genesis; mixed and legacy state refuse to open. Old migrations are never edited in place.                                                    | D0, D5, Phase 4                                | `LINEAGE`                | [T01], [T13], [T14]               |
...
| D6   | Accepted shared Person-read audit shape, operation-specific scope commitments, retention, export position, and audit-before-byte protocol                                                                                                                                               | Route-specific audit tables, repositories, exporters, expiry code, and CLIs                               | Open; retention interval, export position, and commitment-only audit semantics accepted 2026-08-20 for initial V1; D6-1 through D6-3 private structural bodies and the closed unsupported export-capability result green; final fence, audit/retention persistence, opaque-preimage materialization/reproof at the fences, and live removal including the legacy export removals pending |
