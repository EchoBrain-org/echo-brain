...
root user. Set `AWS_PROFILE=echo-prod` for the controller so only the dynamic
...
| `down`      | ECHO AWS deployment authority only; no Cloudflare dashboard action           | Quiesces the application, verifies the exact data mount is unmounted, then updates the persistent slot stack with the host disabled. It removes only EC2, its attachment, and root material, leaving the persistent tunnel, hostname, secret container, retained data volume, and evidence required to recreate the host. If CloudFormation fails while the original host still answers SSM, it attempts and proves a remount and restart of every existing container; otherwise it emits a distinct recovery-required failure. |
| `status`    | Read-only AWS and Cloudflare authority                                       | Reports absent, planned, failed-create, unprotected, incomplete, or ready state. Failed-create and unprotected states do not call Cloudflare and point back to same-stack `slot-init` recovery. A healthy stack checks the exact tunnel, ingress, and DNS record without changing them.                                                                                                                                                                                                                                         |
