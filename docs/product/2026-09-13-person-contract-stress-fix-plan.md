...
The founder accepted the implementation contract when authorizing the review
correction and merge: see
...
process streams, exit, signal and launch-error evidence. The review correction
also validates the complete V2 response, binds actual citations to approved
record/atom/policy evidence, and compares actual citation identities across
repeated attempts. It cannot turn NUL or
...
Still owed: one canonical release and identical Person
