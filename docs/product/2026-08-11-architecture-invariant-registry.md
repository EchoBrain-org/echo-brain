...
from the product architecture documents and records their implementation scope
at code baseline `aaae7509f6b62434b1f23e811b82f3926c38eae3`.
...
supersede A. `INV-12` is the upstream admission rule either can consume.
...
The current reaction surface does not implement that workbench. A proposes a
narrow reviewer-policy approval mode, but it is also not implemented.
...
- lexical/vector retrieval, embeddings, graph-wide interpretation, safe
  statistics, query caches, and retrieval generations;
