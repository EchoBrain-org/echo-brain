...
21. **ADR-OA-021:** The organization administrator HTTPS edge is a separate exact-commit deployable, bundled into neither the employee product nor the single-organization authority. It owns TLS termination, external administrator authentication, request-boundary normalization, route filtering, and authenticated loopback forwarding only; all organization authorization and persistence remain in the authority.
22. **ADR-OA-022:** Administrator-edge access requires both a valid client-certificate chain and an explicit configured SHA-256 allowlist match for that client's subject public-key information. The edge accepts one exact public HTTPS Host/SNI identity, connects only to one bare loopback HTTP authority origin, strips all caller-supplied proxy identity and hop-by-hop material, injects one authenticated proxy credential and privacy-preserving client identity, and fails closed on ambiguous headers, framing, targets, methods, paths, limits, or timeouts.
23. **ADR-OA-023:** The administrator edge requires a canonical bare HTTPS `employee_authority_base_url` as deployment configuration. It answers exact `GET /admin/edge-config` locally with only that non-secret locator so browser-generated invitations target the separate employee-facing authority origin. The endpoint is never forwarded, grants remain browser-local, authority grant registration remains authoritative, and employee enrollment/access routes are never exposed on the administrator hostname.
24. **ADR-OA-024:** The administrator edge has its own exact artifact and external private runtime-file boundary. Local artifact verification, local TLS tests, and rehearsal evidence are non-production development proof; they do not constitute a deployment and cannot close Phase 5 `P5-NET-001`.
