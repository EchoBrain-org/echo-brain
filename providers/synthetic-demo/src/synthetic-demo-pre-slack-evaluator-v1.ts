...
} from "@echo-brain/organization-processing/core/contracts/validation";
import type { ExtractedSignal } from "@echo-brain/organization-processing/core/contracts/decision";
import type { MeetingDocument } from "@echo-brain/organization-processing/core/contracts/meeting";
import {
  AdapterError,
  type AdapterErrorCode,
} from "@echo-brain/organization-processing/core/contracts/adapter";
import type { DecisionProcessorAdapter } from "@echo-brain/organization-processing/core/ports/adapters";
import { syntheticDemoMeetingSourceIdentityV1 } from "./source/synthetic-demo-meeting-source-v1.js";
...
export function northstarExtractionFailureV1(
...
export function northstarEvaluationWithFailureV1(
...
export async function evaluateNorthstarPreSlackExtractionV1(
  ...
  for (const meeting of input.meetings) {
    ...
    try {
      ...
      extractionFailures.push(northstarExtractionFailureV1(meeting.id, "decision_processor.extract", error));
      continue;
    }
    try {
      assertCanonicalDecisionSet(decisions, meeting, input.processor.identity);
    } catch (error) {
      extractionFailures.push(northstarExtractionFailureV1(meeting.id, "canonical_decision_set_validation", error));
      continue;
    }
