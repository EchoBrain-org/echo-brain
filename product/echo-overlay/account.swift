...
final class AccountController: NSObject {
    ...
    private var toolsController: (any ConnectedToolsPresenting)?
    private let makeToolsController: (AccountClient) -> any ConnectedToolsPresenting
    ...
    init(makeToolsController: @escaping (AccountClient) -> any ConnectedToolsPresenting, onSessionWillChange: @escaping () -> Void, mayChangeSession: @escaping () -> Bool, changed: @escaping () -> Void) {
        self.makeToolsController = makeToolsController
    ...
    @objc private func showConnectedTools() {
        ...
        if toolsController == nil { toolsController = makeToolsController(client) }
    ...
    private func allowSessionChange() -> Bool {
        ...
        guard !(toolsController?.hasOutstandingMutation ?? false) else {
            showMessage("Finish the tool change first", detail: "Wait for the pending tool change to finish before changing accounts.")
            return false
        }
        return true
    }
...
// The account shell receives a composed tool surface and never selects a provider.
@MainActor
protocol ConnectedToolsPresenting: AnyObject {
    var hasOutstandingMutation: Bool { get }
...
struct ConnectedTool: Decodable {
    ...
    let external_scope_id: String?
    let external_subject_id: String?
}
...
func decodeConnectedTools(_ data: Data, membershipID: String?) -> ConnectedToolsResult? {
    guard data.count <= 32768,
          ...
          value.result.schema_version == 3, value.result.kind == "echo-organization-person-tools",
          membershipID == value.result.membership_id, value.result.tools.count <= 32
    else { return nil }
    var identifiers = Set<String>()
    for tool in value.result.tools {
        guard tool.tool_id.range(of: "^[a-z][a-z0-9-]{0,63}$", options: .regularExpression) != nil,
              validAccountLabel(tool.display_name, maximum: 128), identifiers.insert(tool.tool_id).inserted,
              ["enabled", "unavailable"].contains(tool.availability),
              ["linked", "unlinked", "revoked", "unavailable"].contains(tool.personal_status) else { return nil }
        for value in [tool.external_scope_id, tool.external_subject_id] {
            if let value, !validAccountLabel(value, maximum: 256) { return nil }
        }
        if tool.availability == "unavailable" {
            guard tool.personal_status == "unavailable", tool.external_scope_id == nil, tool.external_subject_id == nil else { return nil }
        } else {
            guard tool.personal_status != "unavailable" else { return nil }
            if tool.personal_status == "linked" {
                guard tool.external_subject_id != nil else { return nil }
            } else if tool.external_subject_id != nil { return nil }
        }
    }
    return value.result
}
...
func connectedToolsSummary(_ result: ConnectedToolsResult?) -> String {
    ...
    guard !result.tools.isEmpty else { return "Your organization has no supported tools enabled." }
    return result.tools.map { tool in
        guard tool.availability == "enabled" else { return "\(tool.display_name) is not enabled for this organization. Ask an owner to connect it." }
        let status = ["unlinked", "revoked"].contains(tool.personal_status) ? "Not connected" : tool.personal_status
        return "\(tool.display_name) · Organization: enabled (\(tool.external_scope_id ?? ""))\nYour link: \(status)" + (tool.external_subject_id.map { " (\($0))" } ?? "")
    }.joined(separator: "\n\n")
}
