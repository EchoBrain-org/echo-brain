...
test('New project: Create makes it once and opens it, someone is added with Undo, and the files save into it one by one', async () => {
  ...
  // A file dropped before Create waits for the project.
  ...
  await expect(files).toHaveText(['Brief.md · Waiting']);
  ...
  // It opens behind the sheet and joins your projects; the dropped file saves into it.
  ...
  // Add files…: several at once, one after another; one main refuses says why.
  ...
  await expect(files).toHaveText([
    'Brief.md · Saved · Extracting text', 'Notes.txt · Saved · Extracting text', 'Plan.md · Saved · Extracting text',
    'photo.png · Choose a TXT, Markdown, PDF or Word file up to 25 MB.',
  ]);
  expect(uploads().map(call => call.body?.filename)).toEqual(['Brief.md', 'Notes.txt', 'Plan.md']);
