...
const ALLOW_KEYS = ["authority_id", "organization_id", "state_lineage_id", "approval_id", "action", "assignment_version", "final_approver", "selected_policy_id", "policy_contract_sha256", "provider_action_sha256", "decision"] as const;
...
interface Ref {
  ...
  readonly approval_id: string; readonly action: "approve" | "reject"; readonly assignment_version: number;
...
function ref(value: unknown): Ref {
  ...
  for (const key of ["candidate_sha256", "frozen_card_sha256", "approved_snapshot_sha256", "assignment_capability_sha256", "audit_entry_sha256", "provider_action_sha256", "authorization_proof_sha256"] as const) digest(record[key], `resolution ${key}`);
...
function joined(refValue: Ref, witnessValue: Witness, body: Record<string, unknown>): void {
  ...
  for (const key of ["approval_id", "action", "assignment_version", "policy_id", "policy_contract_sha256", "provider_action_sha256"] as const) same(allow[key], refValue[key], `D2 allow ${key}`);
