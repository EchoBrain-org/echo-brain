...
function deterministicCommandId(input: {
  ...
  return `act_${canonicalSha256({
    ...
    // Persisted command IDs retain the original V1 kind value for replay.
...
function publicResult(
  result: Awaited<ReturnType<typeof activatePersonSlackReactionApprovalV2>>,
) {
  return Object.freeze({
...
export async function runCleanPersonSlackReactionApprovalActivateCli(
  ...
  try {
    ...
    const result = await activatePersonSlackReactionApprovalV2({
      ...
      coordinator: new SqlitePersonSlackReactionApprovalActivationCoordinatorV2({
        database,
