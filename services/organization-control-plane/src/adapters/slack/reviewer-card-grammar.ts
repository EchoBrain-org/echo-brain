...
/**
 * The control plane's own reader for the one closed reviewer card grammar.
 *
 * It reconstructs the release draft and the approval presentation from the
 * live message and returns their digests. Title and item text are held only in
 * bounded local memory and are discarded with this call: they are never
 * persisted, logged, traced, measured, or returned.
 */
...
type ReviewerReleaseItemKind = (typeof RELEASE_ITEM_KINDS)[number];

export interface ReviewerCardReconstruction {
...
/**
 * Reconstructs the exact closed reviewer card. Any missing or malformed block,
 * wrong digest, wrong order, mixed namespace, or extra field returns `null`:
 * the card is never partially interpreted.
 */
export function reconstructReviewerCard(input: {
  readonly approval_id: string;
  readonly blocks: readonly unknown[];
  readonly fallback_text: unknown;
}): ReviewerCardReconstruction | null {
  const approvalId = input.approval_id;
  if (!APPROVAL_ID.test(approvalId)) return null;
  const blocks = input.blocks;
  // header + 1..10 items + policy + reaction
  ...
  for (let index = 0; index < itemCount; index += 1) {
    ...
    const digestHex = blockId.slice(prefix.length, blockId.length - "-v1".length);
    ...
    if (!RELEASE_ITEM_KINDS.includes(kind as ReviewerReleaseItemKind)) {
      return null;
    }
  ...
  const consequence = plainTextBlock(
    policyBlock,
    "section",
    `echo-approval-${approvalId}-reviewer-policy-v1`,
    RESTRICTED_REVIEWER_CONSEQUENCE_TEXT.length,
  );
  if (consequence !== RESTRICTED_REVIEWER_CONSEQUENCE_TEXT) return null;
  ...
  return Object.freeze({
    approve_reaction: reactions.approve_reaction,
    reject_reaction: reactions.reject_reaction,
    reviewer_release_draft_sha256: canonicalSha256(draft),
    approval_presentation_sha256: canonicalSha256(presentation),
  });
}

export function isReviewerCardBlockId(
  blockId: string,
  approvalId: string,
): boolean {
  ...
  return (
    suffix === "title-v1" ||
    suffix === "reviewer-policy-v1" ||
    suffix === "reaction-v1" ||
    /^item-\d+-[0-9a-f]{64}-v1$/.test(suffix)
  );
}

export const REVIEWER_CARD_POLICY_ID = RESTRICTED_REVIEWER_POLICY_ID;
