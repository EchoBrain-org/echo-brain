...
import type {
  PersonSlackReactionApprovalActivationCoordinatorV2,
  PersonSlackReactionApprovalActivationFenceV2,
  PersonSlackReactionApprovalActivationResultV2,
  PersonSlackReactionApprovalActivationTransactionV2,
...
export interface StableAuthorityAdministratorFenceV2 {
  withStableAdministratorFence<T>(
    credential: unknown,
    commit: (
      fence: Omit<PersonSlackReactionApprovalActivationFenceV2, "transaction">,
    ) => T,
  ): Promise<T>;
}

export interface SqlitePersonSlackReactionApprovalActivationCoordinatorV2Input {
...
class SqliteActivationTransaction implements PersonSlackReactionApprovalActivationTransactionV2 {
  constructor(
  ...
  activationByCommandId(
    ...
    }) as PersonSlackReactionApprovalActivationResultV2;
  }
  ...
  approvalSurfaceIsEligible(input: {
    ...
    const hasExistingEligibleBinding = rows.some((row) => {
      try {
        const body = validatePersonSlackReactionApprovalBindingContractV2(
