...
export interface CleanPersonSlackApprovalTargetFlagsV1 {
  readonly connection_id: string;
  readonly approval_channel_id: string;
}

export interface SelectedCleanPersonSlackApprovalTargetV1 {
  readonly connection_id: string;
...
/** Reproves the exact current founder link against the one active clean tool. */
export function selectCurrentFounderSlackApprovalTargetV1(
  database: Database.Database,
  state: VerifiedCleanControlPlaneStateV1,
  flags: CleanPersonSlackApprovalTargetFlagsV1,
  owner: { readonly principal_id: string; readonly membership_id: string },
): SelectedCleanPersonSlackApprovalTargetV1 {
  ...
  if (links.length !== 1 || links[0] === undefined) {
    throw new Error(
      "clean approval activation requires one current founder Slack identity link",
    );
  }
  ...
  if (
    ...
    throw new Error("clean approval activation founder Slack link is invalid");
  }
