...
export interface CleanSlackApprovalTokenReaderV1 {
 ...
 */
export class SqliteCleanSlackApprovalTokenReaderV1 implements CleanSlackApprovalTokenReaderV1 {
  constructor(
  ...
  readApprovalToken(input: {
    ...
    if (
      ...
      throw new Error("clean Slack approval connection state is not active");
    }
    ...
    if (
      ...
      throw new Error(
        "clean Slack approval connection state digest is invalid",
      );
    }
    ...
    if (references.length !== 1 || references[0] === undefined) {
      throw new Error(
        "clean Slack approval credential reference is unavailable",
      );
    }
