...
import type { ReprovedFrozenPersonSlackApprovalV2 } from "../application/person-slack-approval-finalization-v2.js";
...
export interface StagePersonSlackPendingApprovalCommandV1 {
  readonly command_id: string;
  readonly approval: ReprovedFrozenPersonSlackApprovalV2;
}

export interface StagedPersonSlackPendingApprovalV1 {
  readonly command_id: string;
...
export class PersonSlackPendingApprovalConflictError extends Error {
  constructor(message = "pending Person Slack approval command conflicts") {
    super(message);
    this.name = "PersonSlackPendingApprovalConflictError";
  }
}
...
function exact(
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("pending Person Slack approval is invalid");
  }
  if (
  ...
  if (
    ...
    throw new Error("pending Person Slack approval has unexpected fields");
  }
...
function text(value: unknown, label: string): string {
  if (
    ...
    throw new Error(`pending Person Slack approval ${label} is invalid`);
  }
  return value;
}
...
export function validateReprovedPersonSlackPendingApprovalV1(
  value: unknown,
): ReprovedFrozenPersonSlackApprovalV2 {
  ...
  if (!APPROVAL_ID.test(approvalId) || record.status !== "pending") {
    throw new Error("pending Person Slack approval identity is invalid");
  }
  ...
  if (
    ...
    throw new Error("pending Person Slack approval policy is invalid");
  }
  for (const field of [
    "connection_contract_sha256",
    "approval_binding_contract_sha256",
  ...
  if (
    ...
    throw new Error(
      "pending Person Slack approval policy commitment is invalid",
    );
  }
  return Object.freeze({
    ...record,
  }) as unknown as ReprovedFrozenPersonSlackApprovalV2;
}
...
function result(
  ...
  idempotent: boolean,
): StagedPersonSlackPendingApprovalV1 {
  return Object.freeze({
...
function storedApproval(row: {
  ...
  if (
    ...
    throw new Error("stored pending Person Slack approval digest is invalid");
  }
  return {
 ...
 */
export function stagePersonSlackPendingApprovalV1(input: {
  readonly database: Database.Database;
  readonly command: StagePersonSlackPendingApprovalCommandV1;
  readonly now: () => string;
}): StagedPersonSlackPendingApprovalV1 {
  if (!COMMAND_ID.test(input.command.command_id)) {
    throw new Error("pending Person Slack approval command_id is invalid");
  }
  ...
  try {
    ...
    if (priorCommand !== undefined) {
      ...
      if (
        ...
        throw new PersonSlackPendingApprovalConflictError();
      }
    ...
    if (priorApproval !== undefined) {
      ...
      if (prior.sha256 !== approvalSha256) {
        throw new PersonSlackPendingApprovalConflictError(
          "pending Person Slack approval ID already names a different approval",
        );
      }
      input.database
        .prepare(
