...
import type {
  PersonSlackApprovalFinalizationCoordinatorV2,
  PersonSlackApprovalFinalizationFenceV2,
  PersonSlackApprovalFinalizationTransactionV2,
...
import { validateReprovedPersonSlackPendingApprovalV1 } from "./sqlite-person-slack-pending-approval-v1.js";
import type Database from "better-sqlite3";

export interface StableAuthorityPersonSlackApprovalFenceV2 {
  withStablePersonSlackApprovalFence<T>(
    commit: (
      fence: Omit<
        PersonSlackApprovalFinalizationFenceV2,
...
export interface SqlitePersonSlackApprovalFinalizationCoordinatorV2Input {
  readonly database: Database.Database;
  readonly authority_fence: StableAuthorityPersonSlackApprovalFenceV2;
}

function parseCanonical(json: string): unknown {
  const parsed = JSON.parse(json) as unknown;
  if (canonicalJson(parsed) !== json) {
    throw new Error("stored Person Slack approval value is not canonical");
  }
  return parsed;
}
...
function frozen<T>(
  ...
  if (canonicalSha256(body) !== sha256) {
    throw new Error("stored Person Slack approval digest is invalid");
  }
...
class SqliteFinalizationTransaction implements PersonSlackApprovalFinalizationTransactionV2 {
  ...
  actionCapability(input: {
    ...
    | FrozenApprovalContractV2<PersonSlackApprovalActionCapabilityV2>
    | undefined {
    const row = this.database
      .prepare(
        `SELECT contract.contract_json, contract.contract_sha256
...
export class SqlitePersonSlackApprovalFinalizationCoordinatorV2 implements PersonSlackApprovalFinalizationCoordinatorV2 {
  constructor(
    private readonly input: SqlitePersonSlackApprovalFinalizationCoordinatorV2Input,
  ) {}

  async withStableProviderHumanAction<T>(
    commit: (fence: PersonSlackApprovalFinalizationFenceV2) => T,
  ): Promise<T> {
    return this.input.authority_fence.withStablePersonSlackApprovalFence(
      (authority) => {
          ...
          const result = commit({
            ...
            reprovedFrozenApprovalById: (approvalId) => {
              ...
              if (canonicalSha256(parsed) !== row.approval_sha256) {
                throw new Error(
                  "stored pending Person Slack approval digest is invalid",
                );
              }
