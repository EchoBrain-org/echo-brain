...
import { SqlitePersonSlackApprovalFinalizationCoordinatorV2 } from "../src/persistence/sqlite-person-slack-approval-finalization-v2.js";
import {
  PersonSlackPendingApprovalConflictError,
  stagePersonSlackPendingApprovalV1,
  type StagePersonSlackPendingApprovalCommandV1,
} from "../src/persistence/sqlite-person-slack-pending-approval-v1.js";
...
describe("clean Person Slack approval finalization", () => {
  it.each(["approve", "reject"] as const)(
    ...
    async (action) => {
      ...
      const first = await finalizePersonSlackApprovalV2(input);
      expect(first).toMatchObject({ kind: "resolved", value: { action } });
      const replay = await finalizePersonSlackApprovalV2(input);
  ...
  it("stages exact commands once and rejects changed command or approval input", () => {
    ...
    const first = stagePersonSlackPendingApprovalV1({
    ...
    ).toThrow(PersonSlackPendingApprovalConflictError);
    expect(() =>
      stagePersonSlackPendingApprovalV1({
        database: fixture.database,
        command: {
...
describe("clean Slack reaction observer", () => {
  ...
  it("observes one matching reaction without a live provider", async () => {
    ...
    const observer = new CleanSlackReactionObserverV1({
      token_reader: { readApprovalToken: () => "private-token" },
      now: () => OBSERVED,
