...
import { activatePersonSlackReactionApprovalV2 } from "../src/application/person-slack-reaction-approval-activation-v2.js";
...
import {
  SqlitePersonSlackReactionApprovalActivationCoordinatorV2,
  type StableAuthorityAdministratorFenceV2,
} from "../src/persistence/sqlite-person-slack-reaction-approval-activation-v2.js";
...
function seed(database: Database.Database) {
  ...
  const eligible = buildPersonSlackReactionApprovalBindingContractV2({
    authority_id: ADMIN.authority_id,
    organization_id: ADMIN.organization_id,
    state_lineage_id: ADMIN.state_lineage_id,
...
describe("private SQLite Person Slack reaction approval activation v2", () => {
  it("reproves current rows and saves one binding, four capabilities, resource, and receipts atomically", async () => {
    ...
    seed(database);
    let sequence = 0;
    const coordinator = new SqlitePersonSlackReactionApprovalActivationCoordinatorV2({
      database,
