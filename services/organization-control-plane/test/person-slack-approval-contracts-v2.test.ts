...
import * as d2 from "../src/application/person-slack-approval-contracts-v2.js";
import {
...
function fixture(
  ...
  const connection = d2.buildOrganizationToolConnectionContractV2({
    ...
    required_provider_scopes: d2.SLACK_APPROVAL_REQUIRED_PROVIDER_SCOPES,
  ...
  const connectionState = d2.buildOrganizationToolConnectionStateV2({
    ...
    observed_granted_scopes: d2.SLACK_APPROVAL_REQUIRED_PROVIDER_SCOPES,
  ...
  const actionCapability = d2.buildPersonSlackApprovalActionCapabilityV2({
    ...coordinates,
...
describe("Person Slack approval D2 contracts", () => {
  ...
  it("rejects missing, extra, reordered, and unsupported body variants", () => {
    ...
    expect(() =>
      d2.validatePersonSlackApprovalBindingContractV2({
    ...
    expect(() =>
      d2.validatePersonSlackApprovalActionCapabilityV2({
  ...
  it("keeps forbidden identity domains and the deferred D3 kind out of every D2 result", () => {
    ...
    const visit = (value: unknown, path: string): void => {
      ...
      if (typeof value === "string") {
        ...
        if (
          d2.SLACK_APPROVAL_REQUIRED_PROVIDER_SCOPES.includes(
            value as (typeof d2.SLACK_APPROVAL_REQUIRED_PROVIDER_SCOPES)[number],
          )
        ) {
          return;
        }
