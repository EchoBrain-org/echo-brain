...
describe("private owner-DM approval stager V1", () => {
  it("binds null policy before publishing one verified-owner DM card", async () => {
    ...
    let assignment = {
      ...
      candidate: {},
      assignment: {
        schema_version: 1, assignment_version: 1,
        current_assignee: { principal_id: "prn_1", membership_id: "mem_1" },
        ...
        assignment_capability_sha256: DIGEST("5"),
      },
      connection_id: "con_1", connection_contract_sha256: DIGEST("6"), connection_state_sha256: DIGEST("7"),
      approval_binding: { approval_binding_id: "bnd_1", approval_binding_contract_sha256: DIGEST("8") },
    ...
    const stage = vi.fn((received: { pending: { canonical_record_policy_id: null }; card_binding: { dm_channel_id: string } }) => {
      operations.push("cp-pending");
      expect(received.pending.canonical_record_policy_id).toBeNull();
    ...
    const stager = new PrivateOwnerDmApprovalStagerV1({
      ...
      assignments: {
        ...
        stage: (received: { approval_binding: { approval_binding_id: string; approval_binding_contract_sha256: `sha256:${string}` } }) => {
          operations.push("assignment");
          assignment = { ...assignment, approval_binding: received.approval_binding };
          return { assignment, created: true };
        },
      } as never,
      control_plane: { stage },
      poster: {
        openDirectMessage: async (user: string) => { operations.push("open-dm"); expect(user).toBe("U01"); return { channel_id: "D01", user_id: "U01" }; },
      ...
      resolve_target: () => ({
        ...
        slack_target: {
          ...
          current_slack_identity_link: assignment.assignment.current_slack_identity_link,
        },
      }) as never,
  ...
  it("uses immutable presentation evidence to tombstone a superseded private DM", async () => {
    ...
    const recovery = {
      assignment: {
        organization_id: "org_1", candidate: {}, assignment: {}, connection_id: "con_1",
        connection_contract_sha256: DIGEST("a"), connection_state_sha256: DIGEST("b"), approval_binding: {},
        dm_channel: { workspace_id: "T01", enterprise_id: null, channel_id: "D01" }, created_at: NOW,
      },
