...
function writeLineageDatabase(
  ...
  try {
    ...
    if (options.omitManifestRow !== true) {
      const body = options.manifestBody ?? databaseManifestBody(role, options.binding);
...
function buildFixture(overrides: FixtureOverrides = {}): string {
  ...
  if (overrides.withRetrievalTree === true) {
    ...
    writeFileSync(
      join(root, 'record-retrieval', 'generations', GENERATION_ID, 'manifest.json'),
      '{}',
    );
    for (const filename of ['facts.sqlite', 'lexical.sqlite', 'content.sqlite']) {
      const role = (
        {
...
describe('state-lineage pre-open guard', () => {
  ...
  it('verifies a retrieval tree and a satisfied active-generation pointer', () => {
    const root = buildFixture({ withRetrievalTree: true, withPointerRow: true });
    const result = verifyStateLineageBeforeOpen(expectation(root));
  ...
  it('refuses duplicated manifest rows', () => {
    ...
    writeLineageDatabase(join(root, 'record-derived.sqlite'), 'record-derived', {
