...
export interface StagePrivateApprovalAssignmentInputV1 {
  ...
  readonly reviewer_target: PrivateSlackApprovalReviewerTargetV1;
 ...
 */
export class SqlitePrivateSlackApprovalAssignmentStateV1 {
  constructor(
  ...
  stage(
    ...
    return this.database.transaction(() => {
      ...
      const target = input.reviewer_target;
      if (target.slack_target.connection.body.organization_id !== organizationId) {
        fail("stage reviewer target does not match Authority metadata");
      }
      if (
        target.slack_target.connection.body.provider_tenant_id !== dm.workspace_id ||
        target.slack_target.connection.body.provider_enterprise_id !== dm.enterprise_id
      ) {
        fail("stage Slack DM does not match the verified workspace");
      }
      assertIdentifier(
        target.slack_target.connection.body.connection_id,
        "stage connection id",
      );
      assertDigest(target.slack_target.connection.sha256, "stage connection contract");
      assertDigest(
        target.slack_target.connection_state.sha256,
      ...
      const currentLink = target.slack_target.current_slack_identity_link;
      ...
      this.database
        ...
        .run(
          ...
          target.slack_target.connection.body.connection_id,
          target.slack_target.connection.sha256,
          target.slack_target.connection_state.sha256,
