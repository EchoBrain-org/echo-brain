import { validateProjectIdV1, validatePersonDocumentIdV1, validatePersonDocumentSearchV1, validatePersonDocumentUploadMetadataV1, validatePersonUpdateRequestId, type PersonDocumentReceiptV1, type PersonDocumentMetadataV1, type PersonDocumentTextV1, type PersonDocumentSearchResultV1 } from '@echo-brain/organization-api';
...
export function createPersonDocumentApplicationV1(dependencies: PersonDocumentApplicationDependenciesV1): PersonDocumentApplicationV1 {
  ...
  return {
    ...
    upload(token, input, bytes): PersonDocumentReceiptV1 { const actor = dependencies.authenticate(token); const metadata = valid(() => validatePersonDocumentUploadMetadataV1(input)); return dependencies.repository.upload(actor, metadata, bytes, () => dependencies.authenticate(token)); },
    status(token, input) { const request_id = valid(() => validatePersonUpdateRequestId(input)); return read(token, { operation: 'status', request_id }) as PersonDocumentMetadataV1; },
