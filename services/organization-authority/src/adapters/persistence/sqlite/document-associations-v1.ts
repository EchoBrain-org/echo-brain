...
type Document = { document_id: string; principal_id: string; membership_id: string; membership_type: string; audience_kind: string; audience_project_id: string | null };
...
export class SqlitePersonDocumentAssociationRepositoryV1 implements PersonDocumentAssociationRepositoryV1 {
  ...
  private relationship(documentId: string): string | null {
    const row = this.database.prepare('SELECT project_id FROM authority_person_document_associations_v1 WHERE document_id=?').get(documentId) as { project_id: string } | undefined;
    ...
    return this.transaction(() => {
      ...
      if (prior) {
        ...
        if (operation === 'associate') {
          ...
          if (existing !== null && existing !== request.project_id) fail();
          if (existing !== null) this.database.prepare('DELETE FROM authority_person_document_associations_v1 WHERE document_id=?').run(request.document_id);
        }
      ...
      const relationship = this.relationship(request.document_id);
      ...
      if (identity(current) !== identity(actor) || canonicalSha256(this.grants(current)) !== canonicalSha256(grants) ||
          this.authorizationRevision(actor.organization_id) !== revision || this.relationship(request.document_id) !== relationship) fail('stale_access_state');
