...
export function assertPersonRequestNamespaceV1(database: Database.Database, actor: AuthorityPersonMembershipBinding, requestId: string, family: 'document' | 'document_association' | 'project' | 'legacy_text'): void {
  const version = database.pragma('user_version', { simple: true }) as number;
  const otherTables = [
    ...(family !== 'legacy_text' ? ['authority_person_updates_v1'] : []),
