...
describe('organization-member-readable protocol', () => {
  it('pins the closed policy digest and complete ordered release card', () => {
    ...
    const card = organizationMemberReadableApprovalPresentation({ draft, approve_reaction: 'white_check_mark', reject_reaction: 'x' });
    ...
    expect(organizationMemberReadablePolicyContractSha256()).toMatch(/^sha256:[0-9a-f]{64}$/);
  ...
  it('rejects extra draft fields and a non-member-readable surface is not implicit', () => {
    ...
    expect(ORGANIZATION_MEMBER_READABLE_RECORD_SURFACE).toBe('slack-organization-member-readable-v1');
  });
});
