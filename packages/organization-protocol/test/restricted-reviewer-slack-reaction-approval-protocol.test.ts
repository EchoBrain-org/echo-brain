...
describe("restricted-reviewer Slack-reaction policy release and presentation", () => {
  it("projects the complete ordered approval card", () => {
    const draft = projectRestrictedReviewerReleaseDraft({
  ...
  it("rejects malformed release data before it reaches a presentation", () => {
      ...
      projectRestrictedReviewerReleaseDraft({
        ...
        brief: {
          ...
          meeting: {
            id: "meeting-1",
            title: "x".repeat(
              MAX_RESTRICTED_REVIEWER_CARD_TITLE_SCALARS + 1,
            ),
          },
        },
      }),
    ...
    expect(() =>
      validateRestrictedReviewerReleaseDraft({ ...draft, extra: true }),
    ).toThrow(OrganizationProtocolValidationError);
  });
});
