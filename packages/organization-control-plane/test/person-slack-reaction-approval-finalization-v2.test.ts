...
import {
  PersonSlackReactionApprovalFinalizationConflictError,
  PersonSlackReactionApprovalFinalizationDeniedError,
  finalizePersonSlackReactionApprovalV2,
  type PersonSlackReactionApprovalFinalizationCoordinatorV2,
  type PersonSlackReactionApprovalFinalizationFenceV2,
  type PersonSlackReactionApprovalFinalizationResultV2,
...
} from "../src/application/person-slack-reaction-approval-activation-v2.js";
...
class FakeCoordinator implements PersonSlackReactionApprovalFinalizationCoordinatorV2 {
  ...
  readonly capabilities = new Map<string, FrozenApprovalContractV2<ReturnType<typeof buildPersonSlackReactionApprovalActionCapabilityV2>>>();
  connection;
  connectionState;
  readonly binding;
  readonly link;
  ...
  async withStableProviderHumanAction<T>(
    ...
    this.fenceCount += 1;
    this.inFence = true;
    const transaction: PersonSlackReactionApprovalFinalizationTransactionV2 = {
...
class FakeProvider {
  ...
  outcome: PersonSlackReactionApprovalProviderResultV2["kind"] = "observed";
  resultOverride: unknown | undefined;
  afterObserve: (() => void) | undefined;
  expectations: PersonSlackReactionApprovalProviderExpectationV2[] = [];
  ...
  async observeApprovalReaction(
    ...
    if (this.resultOverride !== undefined) {
      return this.resultOverride as PersonSlackReactionApprovalProviderResultV2;
    }
...
function resolved(
  result: PersonSlackReactionApprovalFinalizationResultV2,
): ProviderHumanActionDurableResult {
  expect(result.kind).toBe("resolved");
  return (result as Extract<PersonSlackReactionApprovalFinalizationResultV2, { kind: "resolved" }>).value;
}

describe("private Person Slack reaction approval finalization v2", () => {
  ...
  it("binds recovery to the requested approval and the verified audit chain", async () => {
    ...
    ).rejects.toBeInstanceOf(PersonSlackReactionApprovalFinalizationDeniedError);
