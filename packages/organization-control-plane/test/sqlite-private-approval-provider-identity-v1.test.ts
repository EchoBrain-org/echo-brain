...
function setup() {
  ...
  const connection = buildOrganizationToolConnectionContractV2({
    authority_id: "oau_00000000-0000-4000-8000-000000000001", organization_id: pending.organization_id, state_lineage_id: "lineage-1", connection_id: "con_00000000-0000-4000-8000-000000000001", provider_issuer: "https://slack.com", provider_tenant_kind: "workspace", provider_tenant_id: "T01234567", provider_enterprise_id: null, tool_kind: "slack", provider_app_id: "A01234567", provider_bot_id: "B01234567", provider_bot_user_id: "U09876543", required_provider_scopes: SLACK_ORGANIZATION_TOOL_REQUIRED_SCOPES, public_connection_configuration_sha256: sha("f"),
  });
  const connectionSha = canonicalSha256(connection);
  const state = buildOrganizationToolConnectionStateV2({
    connection_id: connection.connection_id, connection_contract_sha256: connectionSha, connection_status: "active", credential_reference_sha256: sha("0"), observed_granted_scopes: SLACK_ORGANIZATION_TOOL_REQUIRED_SCOPES, verification_event_id: "evt_00000000-0000-4000-8000-000000000001", verification_evidence_sha256: sha("1"), verification_revision: 1, verified_at: now(),
  });
